...
class WeatherRandomAPI : ObservableObject {
    @Published var weather: Weather = Weather(latitude: 0.0, longitude: 0.0, current_weather: CurrentData(time: "", temperature: 0.0))
    ...
    func loadData(completion:@escaping (Weather) -> ()) {
        ...
        URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            DispatchQueue.main.async {
                completion(weather ?? Weather(latitude: 0.0, longitude: 0.0, current_weather: CurrentData(time: "", temperature: 0.0)))
            }
        }.resume()
    }
}
