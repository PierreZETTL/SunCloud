...
class WeatherRandomAPI : ObservableObject{
    ...
    func loadData(completion:@escaping (Weather) -> ()) {
        guard let url = URL(string: "https://api.open-meteo.com/v1/forecast?latitude=\(CLLocationDegrees(Float.random(in: 0...50)))&longitude=\(CLLocationDegrees(Float.random(in: 0...50)))&hourly=temperature_2m,rain,cloudcover,snowfall&daily=temperature_2m_max,temperature_2m_min,rain_sum,sunrise,sunset,snowfall_sum&current_weather=true&timezone=Europe%2FParis") else {
            print("URL invalide.")
            return
        }
